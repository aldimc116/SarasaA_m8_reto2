...
trend <- df %>%
  filter(!is.na(happy) & !is.na(health)) %>%
  group_by(health) %>%
  summarise(mean_happy = mean(as.numeric(happy), na.rm = TRUE)) # Usar as.numeric

# Heatmap con tendencia
ggplot(df_heatmap, aes(x = health, y = happy, fill = prop)) + # Usar factores directamente
  geom_tile(color = "white", alpha = 0.9) +
  scale_fill_gradient(low = "lightblue", high = "darkblue") +
  geom_text(aes(label = scales::percent(prop, accuracy = 1)), color = "white", size = 3) +
  geom_line(
    data = trend,
    aes(x = health, y = mean_happy, group = 1), # group = 1 para la línea de tendencia sobre factores
    color = "red", size = 1,
    inherit.aes = FALSE
  ) +
  geom_point(
...
df_ridges <- df %>%
  ...
  mutate(
    ...
    happy_numeric = as.numeric(happy) # Usar la versión numérica para el eje x del ridge
  )

trend1 <- df_ridges %>%
  group_by(age_group) %>%
  summarise(mean_happy = mean(happy_numeric),
            mid_age_pos = as.numeric(factor(age_group))) # Posición para el suavizado
...
df_ridges2 <- df %>%
  ...
  mutate(
    age_group = cut(agea, breaks = seq(15, 90, by = 5), include.lowest = TRUE, right = FALSE),
    health_numeric = as.numeric(health) # Usar la versión numérica para el eje x del ridge
  )
...
p1 + p2 +
  ...
  theme(plot.title = element_text(size = 14, hjust = 0.5)) # Título global para patchwork
```
...
p_gender_health <- ggplot(df_prop4, aes(x = gndr, y = prop, fill = factor(health))) + # Usar gndr y health directamente como factores
  ...
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 12), # Ajustar tamaño del título para el plot combinado
...
# Combinar los plots con un título global
...
p_birth_happy <- ggplot(df_prop1, aes(x = brncntr, y = prop, fill = factor(happy))) + # Usar brncntr y happy directamente
geom_bar(stat = "identity", position = "fill", color = "white") +
scale_y_continuous(labels = scales::percent_format()) +
...
p_birth_health <- ggplot(df_prop2, aes(x = brncntr, y = prop, fill = factor(health))) + # Usar brncntr y health directamente
geom_bar(stat = "identity", position = "fill", color = "white") +
scale_y_continuous(labels = scales::percent_format()) +
scale_fill_viridis_d(option = "plasma", name = "Nivel de salud percibida") +
labs(x = "Origen", y = "Proporción",
...
theme(plot.title = element_text(hjust = 0.5, size = 14)) # Título global para el conjunto
```
